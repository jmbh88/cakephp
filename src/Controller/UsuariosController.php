...
class UsuariosController extends AuthController{
    ...
    public function login() {
        if($this->request->is('post')) {
            $user = $this->Auth->identify(); //recupero el usuario
            if($user) {
                $this->Auth->setUser($user);
                return $this->redirect($this->Auth->redirectUrl()); //con esta linea me dirige a la pagina declara en auth : loginredirect
            }
            $this->Flash->error('Usuarioy/o clave invalido');           
        }
        $this->viewBuilder()->layout('public');
    }
    
    public function logout() {
        return $this->redirect($this->Auth->logout()); //invalida la sesion y dirige al logoutredirect
    }
